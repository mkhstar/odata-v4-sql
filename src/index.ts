...
import { filter, query } from "../odata-v4-parser/src/parser";
...
export function createQuery(
  ...
  let ast: Token = <Token>(
    (typeof odataQuery == "string" ? query(<string>odataQuery) : odataQuery)
  );
...
export function createFilter(
  ...
  if (typeof type != "undefined" && type) options.type = type;
  let ast: Token = <Token>(
    (typeof odataFilter == "string" ? filter(<string>odataFilter) : odataFilter)
  );
  return new Visitor(options).Visit(ast).asType();
}
